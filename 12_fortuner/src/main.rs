...
struct Args {
    ...
    #[arg(short, long="insensitive", action=ArgAction::SetTrue, help="Ignore case for -m patterns")]
    insensitive: bool,
...
fn run(args: Args) -> Result<()> {
    ...
    if let Some(pattern) = pattern {
        ...
        println!(
            "{}",
            pick_fortune(&fortunes, args.seed).unwrap_or("No fortunes found".to_string())
        )
    }
